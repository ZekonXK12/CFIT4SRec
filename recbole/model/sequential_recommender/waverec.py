...
class WaveRec(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        self.fc=nn.Linear(3*self.hidden_size, 64)
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        # low_freq_component = self.upsampler(low_freq_component)
        # high_freq_component = self.upsampler(high_freq_component)
        # fused = torch.concat([input_emb, low_freq_component, high_freq_component], dim=2)
        # fused = self.fc(fused)

        stacked = torch.stack([input_emb, low_freq_component, high_freq_component], dim=-1)
